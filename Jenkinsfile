pipeline {
    ...
    environment {
        PROD_ENV     = "/opt/env/.env.gigawat"
        IMAGE_NAME   = "gigawat"
        TEST_TAG     = "test"
        PROD_TAG     = "latest"
        CONTAINER_DB = "gigawat_db_test"
        CONTAINER_WEB = "gigawat_web_test"
        CONTAINER_REDIS = "gigawat_redis_test"
        STACK_NAME = "gigawat"
    }
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                git branch: 'main', credentialsId: 'ssh', url: 'git@github.com:JscorpTech/gigawat.git'
            }
        }
        stage('Prepare') {
            steps {
